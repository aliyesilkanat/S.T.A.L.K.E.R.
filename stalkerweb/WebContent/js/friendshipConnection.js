...
d3.json("/FriendshipConnectionServlet?userURI="
		+ userURI , function(error, graph) {
...
node.append("title")
  .text(function(d) { return d.UserUri; }); 
...
function threshold(thresh) {
    data.links.splice(0, data.links.length);
		...
		for (var i = 0; i < datarec.links.length; i++) {
			if (datarec.links[i].value > thresh) {data.links.push(datarec.links[i]);}
		}
    restart();
}
