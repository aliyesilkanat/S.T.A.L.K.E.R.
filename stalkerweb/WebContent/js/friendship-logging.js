...
function LineChart(_startDate, _endDate, _data){
	...
	function formatDate(date){
		var out = (date.getDate() + 1) + "." + (date.getMonth() + 1	);
		return out.toString();
	}
	...
	function init(_data){
			...
			.range([height, 0]);
			canvas = d3.select("body").append("svg").attr("width", outlineSize + width).attr("height", outlineSize + height);
	}
	...
	var createAxisX = function(canvas, ticks) {
		...
		var dateTickGap = (endDate - startDate)/ticks;
		var tickGap = (xMaxValue - xMinValue)/ticks;
		for(i = 1; i < ticks; i++){
			xAxis.append("text")
					.attr("x", xScale(i * tickGap))
					.attr("y", height + 15)
					.attr("fill", xAxisTextColor)
					.text(formatDate(new Date(i * dateTickGap + startDate)));
		}
	}
	...
	var drawLineChart = function(canvas, data, color) {
	...
	this.removeChart(chart) {
		...
		else chartToBeRemoved = chart;
		
		var d3Obj = d3.select(chartToBeRemoved);
		if(d3Obj != null)
			d3Obj.remove();
		var index = charts.indexOf(chartToBeRemoved);
		if(index > -1)
